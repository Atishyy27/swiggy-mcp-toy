...
// Tools that move money. We do not call these, ever. They are counted separately
// so the coverage number stays honest instead of quietly rounding up.
const SPENDS_MONEY = new Set(["place_food_order", "checkout"]);
...
async function probe(server) {
  ...
  const call = async (name, args = {}) => {
    const t = textOf(await client.callTool({ name, arguments: args }));
    await sleep(250);
    return t;
  };
  ...
  try {
    const t = await call("get_addresses", { page: 1, pageSize: 10 });
    addressId = (t.match(/\(ID:\s*(\w+)\)/) || [])[1] || "";
    const [s, n] = judge(t);
    record(server, "get_addresses", s, addressId ? `addressId ${addressId}` : n);
  } catch (e) {
    record(server, "get_addresses", "err", e.message);
  }

  // Everything the server offers, minus what we already did, minus what costs money.
  const todo = listed.map((t) => t.name).filter((n) => n !== "get_addresses");
  ...
  for (const tool of todo) {
    ...
    try {
      ...
      const rule = /not eligible|does not exist|not applicable|minimum|expired|not serviceable/i.test(msg);
